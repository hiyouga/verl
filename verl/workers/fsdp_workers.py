...
class ActorRolloutRefWorker(Worker):
    ...
    def update_actor(self, data: DataProto):
        ...
        if self._is_offload_param:
            load_fsdp_param_and_grad(
                module=self.actor_module_fsdp, device_id=torch.cuda.current_device(), load_grad=self._is_offload_grad
            )
        if self._is_offload_optimizer:
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        if self._is_offload_param:
            load_fsdp_param_and_grad(
                module=self.ref_module_fsdp, device_id=torch.cuda.current_device(), load_grad=self._is_offload_grad
            )
...
class CriticWorker(Worker):
    ...
    def compute_values(self, data: DataProto):
        ...
        if self._is_offload_param:
            load_fsdp_param_and_grad(
                module=self.critic_module, device_id=torch.cuda.current_device(), load_grad=self._is_offload_grad
            )
