...
class RayPPOTrainer(object):
    ...
    def _save_checkpoint(self):
        actor_local_path = os.path.join(
            self.config.trainer.default_local_dir, "actor", f"global_step_{self.global_steps}"
        )
        actor_remote_path = (
            None
        ...
        if self.use_critic:
            critic_local_path = os.path.join(
                self.config.trainer.default_local_dir, "critic", f"global_step_{self.global_steps}"
            )
            critic_remote_path = (
                None
                if self.config.trainer.default_hdfs_dir is None
